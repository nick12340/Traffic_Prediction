...
class spatio_conv_layer(nn.Module):
    ...
    def forward(self, x):
        ...
        # if(x.device!=self.Lk.device):
        #     if x.device=='cuda:0':
        #         self.Lk = self.Lk.to(torch.device('cuda:0'))
        #     else:
        #         self.Lk = self.Lk.to(torch.device('cuda:1'))
...
class st_conv_block(nn.Module):
    ...
    def forward(self, x):
        ...
        x_s2 = self.sconv2(x_s1)+x_t1
        x_s3 = self.sconv3(x_s2)
        x_s4 = self.sconv4(x_s3)+x_s2
        x_t2 = self.tconv2(x_s4)
